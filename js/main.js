...
//   function Bird(name) {
//     this.name = name;
//     Animal.call(this, "Bird"); // Call Animal constructor
//   }
...
//   eagle.move(); // Output: Bird is moving
...
async function fetchWeather(query){
    ...
    const response=await fetch(`https://api.weatherapi.com/v1/forecast.json?key=af193c4729e043b0ab5141746231907&q=${query}&days=3`);
...
function updateDOM(data) {
    ...
    today.querySelector("#symbol-icon-main").src=current.condition.icon;
    ...
    today.querySelector("#humitity").childNodes[1].nodeValue=current.humidity + "%"
    today.querySelector("#wind-speed").childNodes[1].nodeValue=current.wind_kph +"km/h"
    ...
    for(var i=0; i<next_day.length; i++){
      ...
      next_day[i].querySelector(".month").innerHTML = formattedDate1[1] + ' ' + formattedDate1[2]; // Date and Month (e.g., 30 November)

      next_day[i].querySelector(".symbol-icon").src=forcast[i+1].day.condition.icon;
      next_day[i].querySelector(".degree").innerHTML=forcast[i+1].day.maxtemp_c+"°C"
      next_day[i].querySelector(".state").innerHTML=forcast[i+1].day.condition.text
      next_day[i].querySelector(".state").style.color=textColor
      next_day[i].querySelector(".wind-dir").childNodes[1].nodeValue=windDir(current.wind_dir)
      next_day[i].querySelector(".humitity").childNodes[1].nodeValue=forcast[i+1].day.avghumidity + "%"
      next_day[i].querySelector(".wind-speed").childNodes[1].nodeValue=forcast[i+1].day.maxwind_kph +"km/h"
...
function windDir(currentDirection){
    ...
    if(currentDirection == "N"|| currentDirection == "NbE"||currentDirection == "NNE"||currentDirection == "NEbN"){
        return "North" 
    }
    if(currentDirection == "NE"||currentDirection == "NEbE"||currentDirection == "ENE"||currentDirection ==="EbN"){
        return "North East"}
    if(currentDirection == "E"||currentDirection == "EbS"||currentDirection == "ESE"||currentDirection ==="SEbE"){
        return "East"}
    if(currentDirection == "SE"||currentDirection == "SEbS"||currentDirection == "SSE"||currentDirection ==="SbE"){
        return "South East"}
    if(currentDirection == "S"||currentDirection == "SbW"||currentDirection == "SSW"||currentDirection ==="SWbS"){
        return "South"}
    if(currentDirection == "SW"||currentDirection == "SWbW"||currentDirection == "WSW"||currentDirection ==="WbS"){
        return "South West"}
    if(currentDirection == "W"||currentDirection == "WbN"||currentDirection == "WNW"||currentDirection ==="NWbW"){
        return "West"}
    if(currentDirection == "NW"||currentDirection == "NWbN"||currentDirection == "NNW"||currentDirection ==="NbW"){
        return "North West"}
...
//             videoSrc="assets/staicmobilbg.mp4";
//             videoElement.load();
//             console.log("updated videoSrc:" ,videoSrc);
//             document.getElementById("video_src").setAttribute('src',videoSrc);
